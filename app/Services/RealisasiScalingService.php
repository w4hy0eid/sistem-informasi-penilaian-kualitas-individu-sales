...
class RealisasiScalingService
{
    ...
    public function create(CreateRealisasiRequest $request)
    {
        ...
        $result = RealisasiScaling::create([
            'month' => $request->month,
            'value' => $request->value,
    ...
    {
        $sustain = RealisasiScaling::find($id);
        ...
        // dd($sisaPembayaran, $sales->nilai_project, $sumRealisasi);
    ...
    {
        $result = RealisasiScaling::where('user_id', $userId)->where("sales_id", $salesId)->get();
    ...
    {
        $result = RealisasiScaling::where('id', $idUser)->first();
