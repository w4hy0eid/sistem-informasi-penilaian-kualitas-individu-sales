...
function newRealisasi() {
    $("#titleModal").text("Create Realisasi scaling");
    $("#formRealisasi")[0].reset();
    statusForm = "create";
    $("#alertMessage").empty();
}

function updateScaling(userId, salesId) {
    statusForm = "update";
    $("#alertMessage").empty();
    $.getJSON(baseUrl + `/r-scaling/user/${userId}/sales/${salesId}`, (data) => {
        let html = "";
        $("#listTitle").text("Update Realisasi Scaling");
...
function detail(id) {
    ...
    link.href = baseUrl + `/view/realisasi-scaling-detail/${id}?page=scaling`;
    link.target = "_blank";
    link.click();
}
...
function deleteData(idUser) {
    if (confirm("hapus?")) {
        $.ajax({
            url: baseUrl + `/action/realisasi/scaling/delete/${idUser}`,
            type: "delete",
            dataType: "json",
            data: {
                _token: tokenCsrf,
...
$("#submitFormCreate").click(function () {
    ...
    const url =
        statusForm === "create"
            ? "/action/realisasi/scaling/create"
            : `/action/realisasi/scaling/update/${userId}`;
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (statusForm === "create") {
                ...
                location.replace(baseUrl + '/view/realisasi-scaling');
            }
        },
