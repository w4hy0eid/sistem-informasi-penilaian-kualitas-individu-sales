...
function newTargetUpload() {
    $('#uploadtarget').attr('action', baseUrl + '/action/import-target/scaling');
}

function newTarget() {
    $("#titleModal").text("Create Target Scaling");
    $('#formTarget')[0].reset();
    statusForm = 'create';
    $("#alertMessage").empty();
}
...
function updateUserBtn(idUser) {
    statusForm = 'update';
    $("#alertMessage").empty();
    $.getJSON(baseUrl + '/target/scaling/' + idUser, (data) => {
        $("#exampleModalLabel").text("Update Target Scaling");
...
function deleteScaling(idUser) {
    if(confirm("hapus?")) {
        $.ajax({
            url: baseUrl + `/action/target/scaling/delete/${idUser}`,
            type: "delete",
            dataType: "json",
            data: {
                "_token": tokenCsrf
...
$("#submitFormCreate").click(function () {
    ...
    const url = statusForm === 'create' ? '/action/target/scaling/create' : `/action/target/scaling/update/${userId}`;
