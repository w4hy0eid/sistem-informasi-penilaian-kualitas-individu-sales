...
function newTargetUpload() {
    $('#uploadtarget').attr('action', baseUrl + '/action/import-target/sustain');
}

function newTarget() {
    $("#titleModal").text("Create Target Sustain");
    $('#formTarget')[0].reset();
    statusForm = 'create';
    $("#alertMessage").empty();
}
...
function updateUserBtn(idUser) {
    statusForm = 'update';
    $("#alertMessage").empty();
    $.getJSON(baseUrl + '/target/sustain/' + idUser, (data) => {
        $("#exampleModalLabel").text("Update Target Sustain");
...
function deleteSustain(idUser) {
    if(confirm("hapus?")) {
        $.ajax({
            url: baseUrl + `/action/target/sustain/delete/${idUser}`,
            type: "delete",
            dataType: "json",
            data: {
                "_token": tokenCsrf
...
$("#submitFormCreate").click(function () {
    ...
    const url = statusForm === 'create' ? '/action/target/sustain/create' : `/action/target/sustain/update/${userId}`;
